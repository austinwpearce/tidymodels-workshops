...
augment(lr_fit, new_data = taxi_train) %>%
  roc_curve(truth = tip, .pred_yes) |> 
  autoplot()
```
...
```{r}
augment(x = taxi_fit, new_data = taxi_train)
```
...
taxi_fit |> 
  augment(taxi_train) |> 
  brier_class(truth = tip, .pred_yes)
```
...
taxi_mc <- mc_cv(taxi_train)

fit_resamples(taxi_wflow, taxi_mc) |> 
  collect_metrics()
```
...
# BEWARE as 1000 bootstraps could take forever to run
# set.seed(3214)
# taxi_boot <- bootstraps(taxi_train)
taxi_res <- fit_resamples(rf_wflow, taxi_mc)

collect_metrics(taxi_res)
```
