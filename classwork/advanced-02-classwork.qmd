...
```{r}
# Your code here!
summary(hotel_rec) |> unnest(type)
```

## A base recipe

Preprocess the data and get it ready for the modeling workflow.
...
# Your code here!
hotel_train |> 
  recipe(avg_price_per_room ~ .) |> 
  step_YeoJohnson(lead_time) |>
  step_dummy(all_factor_predictors()) |> # may be a problem because of agents (98!)
  step_zv(all_predictors())
```
...
```{r}
reg_metrics <- metric_set(rsq, mae)
```
